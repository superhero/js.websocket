...
module.exports = (options = {}) =>
{
  ...
  ({
    ...
    {
      const dto = JSON.stringify({event:event, data:data});

      (gl0bal ? sockets : [socket]).forEach(
        (socket) => setImmediate(() =>
        {
          for(let i = 0, t = 1000; i < dto.length; i += t)
          {
            const enc = encode(dto.substring(i, t));
