  ...
  server.on('connection', socket =>
  {
    ...
    socket.once('data', headers =>
    {
        ...
        try
        {
          ...
          for(const msg of decode(dataCache))
          {
            ...
            else
            {
              ...
              dto.event != 'ping'
              && debug('recived message:', dto);

              setImmediate(() => events.emit(dto.event, context, dto.data));
            }
          }
      ...
      socket.on('close', () =>
      {
        ...
        config.onClose && config.onClose(context);
      });

      // when an event is emitted, this is the handle that is past on
      // as the first argument to the observer listening to that event.
      const context =
      {
        headers       : headers,
        ...
        {
          const
          log = `emitted: "${event}", global: "${globaly}"`,
          enc = encode(JSON.stringify({event, data}));
