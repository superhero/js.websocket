...
module.exports = class
{
  constructor(options)
  {
    ...
    for(let event of ['close','connection','listening','error'])
      this.server.on(event, () => this.log('server:', event));
  ...
  onConnection(socket)
  {
    ...
    socket.emit = this.emit.bind(this, socket);
  ...
  onData(socket, data)
  {
    ...
    socket.buffer = Buffer.concat([socket.buffer, data].filter(_ => _));
